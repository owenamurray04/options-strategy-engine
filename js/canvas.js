/* Prediction canvas: the probability cloud over price × time, drawn on ONE <canvas> in a single
   coordinate system (density + grid + axes + history + dot + expiry marker). Owns the view state
   (mode + per-side spreads) and all interaction. Exposes getView() for the engine, plus
   applyStance()/setActiveExp() for the stepper to drive. */
...
let uM = 0.70, vM = 0.62, sR = 0.13, sL = 0.085, sU = 0.13, sD = 0.085, t3d = 0, animating = false, yawM = 0, pitchM = 0;
let curExp = 'aug';
const dMin = -60, dMax = 95, dTot = dMax - dMin, fNow = (0 - dMin) / dTot;
function clamp(x, a, b) { return Math.max(a, Math.min(b, x)); }
function price(v) { return 255 + v * 250; }
...
const GL = [300, 350, 400, 450];
function drawPlane(cf, sf, cy, sy) {
  ctx.lineWidth = 1; ctx.strokeStyle = 'rgba(150,150,150,0.28)';
  for (let g = 0; g < GL.length; g++) { const v = (GL[g] - 255) / 250; const a = proj(0, v, 0, cf, sf, cy, sy), b = proj(1, v, 0, cf, sf, cy, sy); ctx.beginPath(); ctx.moveTo(a.x, a.y); ctx.lineTo(b.x, b.y); ctx.stroke(); }
...
function drawTop(cf, sf, cy, sy) {
  ...
  for (let g = 0; g < GL.length; g++) { const v = (GL[g] - 255) / 250; const pp = proj(0, v, 0, cf, sf, cy, sy); ctx.fillText('$' + GL[g], pp.x - 6, pp.y); }
  ...
  /* selected-expiration marker — ties the engine's chosen expiry to the time axis */
  const ue = clamp((MKT.exp[curExp].dte + 60) / dTot, 0, 1);
  ...
  ctx.fillStyle = 'rgba(24,95,165,0.95)'; ctx.textAlign = 'center'; ctx.textBaseline = 'bottom'; ctx.fillText('Exp ' + MKT.exp[curExp].label, e2.x, e2.y - 3);
  /* calibrated confidence bands at the expiration: pull-to-edge = 1σ ⇒ ±1σ = 68%, ±2σ = 95%.
     These are exactly the split-normal the engine integrates against, so what you see = what it ranks. */
...
function readouts() {
  ...
  document.getElementById('cImp').textContent = 'Reads as: by ' + MKT.exp[curExp].label + ' you’re ~68% sure of the inner band, ~95% of the outer. Leaning ' + ps + ', ' + ts + ' — ' + imp + '.';
}
...
let pend = false; function reqDraw() { if (pend) return; pend = true; requestAnimationFrame(function () { pend = false; draw(); }); }
...
/* ---- public API ---- */
export function applyStance(stance) {
  if (stance === 'up') { vM = 0.62; sU = 0.13; sD = 0.085; }
  else if (stance === 'down') { vM = 0.34; sU = 0.085; sD = 0.13; }
  else if (stance === 'flat') { vM = 0.48; sU = 0.07; sD = 0.07; }
  draw();
}
export function setActiveExp(key) { curExp = key; draw(); }
/* The user's drawn view, handed to the engine in dollar units. */
export function getView() { return { modePrice: price(vM), sigUp: Math.max(8, sU * 250), sigDn: Math.max(8, sD * 250) }; }
